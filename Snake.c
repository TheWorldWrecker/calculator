...
GRAPHDATA filledGraph = {3, 3, (unsigned char*) "\xE0\xE0\xE0"};
...
int placeFood() {
    ...
    x = emptyCells[rand() % emptyCount][0];
    y = emptyCells[rand() % emptyCount][1];
    board[x][y] = FOOD;
}
...
void drawSnake() {
    ...
    for(i = 0; i < snakeLen-1; ++i) {
        Bdisp_ClearLineVRAM(
            snakePos[i][0]*3 + 46, snakePos[i][1]*3 + 3,
            snakePos[i+1][0]*3 + 46, snakePos[i+1][1]*3 + 3);
    }
}
...
int drawScreen() {
    ...
    Bdisp_DrawLineVRAM(43, 63, 121, 63);
    Bdisp_DrawLineVRAM(121, 63, 121, 0);
    Bdisp_DrawLineVRAM(121, 0, 43, 0);
    Bdisp_DrawLineVRAM(44, 1, 44, 62);
    Bdisp_DrawLineVRAM(44, 62, 120, 62);
    Bdisp_DrawLineVRAM(120, 62, 120, 1);
    Bdisp_DrawLineVRAM(120, 1, 44, 1);
    drawSnake();
    ...
    for(i = 0; i<64; ++i) {
        Bdisp_SetPoint_VRAM(42, i, i%2);
        Bdisp_SetPoint_VRAM(122, i, !(i%2));
    }
...
int frame() {
    const nextFrameTime = speed == SPEED_HI ? 90
     : speed == SPEED_MD ? 135 : 200;
    int i, j;
